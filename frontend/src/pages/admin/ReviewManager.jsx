...
import { FiMessageCircle, FiEye, FiEyeOff, FiStar, FiFileText, FiBox, FiTrash2 } from 'react-icons/fi';
        ...
        <div className="font-sans pb-10 max-w-6xl mx-auto">
            <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1 text-sm font-medium">Kiểm duyệt và ghim các đánh giá nổi bật để hiển thị cho khách hàng.</p>
                </div>
            </div>
            ...
            <div className="flex gap-4 border-b border-slate-200 mb-6">
                <button 
                    ...
                    className={`flex items-center gap-2 pb-3 px-2 font-bold transition-all ${
                        activeTab === 'products' ? 'text-sky-600 border-b-2 border-sky-600' : 'text-slate-500 hover:text-slate-700'
                    }`}
                >
                    <FiBox size={18} /> Đánh giá Món ăn ({productReviews.length})
                </button>
                <button 
                ...
                >
                    <FiFileText size={18} /> Bình luận Bài viết ({articleComments.length})
                </button>
            </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-100 text-slate-500 text-xs uppercase tracking-wider">
                                        ...
                                        <th className="p-4 font-bold text-center w-40">Thao tác</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {productReviews.length === 0 ? (
                                        <tr><td colSpan="4" className="text-center p-8 text-slate-500">Chưa có đánh giá nào.</td></tr>
                                    ) : productReviews.map((review) => {
                                        const displayAvatar = (review.user && review.user.avatar) ? review.user.avatar : review.avatar;
                                        return (
                                        <tr key={review._id} className={`border-b border-slate-50 hover:bg-slate-50 transition-colors ${review.isHidden ? 'opacity-60 bg-slate-100/50' : ''}`}>
                                            <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                    <img src={displayAvatar ? getImageUrl(displayAvatar) : `https://ui-avatars.com/api/?name=${review.name}&background=E0F2FE&color=0284C7`} className="w-10 h-10 rounded-full object-cover shadow-sm" alt={review.name} />
                                                    <div>
                                                        <p className="font-bold text-slate-800 text-sm">{review.name}</p>
                                                        <p className="text-xs text-slate-400">{new Date(review.createdAt).toLocaleDateString('vi-VN')} - {new Date(review.createdAt).toLocaleTimeString('vi-VN', {hour: '2-digit', minute:'2-digit'})}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-4">
                                            ...
                                            <td className="p-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm font-semibold text-sky-600 truncate max-w-[120px]" title={review.product.name}>{review.product.name}</span>
                                                </div>
                                            </td>
                                            <td className="p-4">
                                                <div className="flex items-center justify-center gap-2">
                                                    <button 
                                                        ...
                                                        className={`p-2 rounded-lg transition-colors ${review.isHidden ? 'bg-slate-200 text-slate-600 hover:bg-slate-300' : 'bg-sky-50 text-sky-600 hover:bg-sky-100'}`}
                                                        title={review.isHidden ? 'Hiện đánh giá' : 'Ẩn đánh giá'}
                                                    >
                                                        {review.isHidden ? <FiEyeOff size={18} /> : <FiEye size={18} />}
                                                    </button>
                                                    
                                                    <button 
                                                        onClick={() => handleToggleProductReviewPin(review.product._id, review._id)}
                                                        className={`p-2 rounded-lg transition-colors ${review.isPinned ? 'bg-amber-100 text-amber-600 hover:bg-amber-200' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                                                        title={review.isPinned ? 'Bỏ ghim' : 'Ghim lên đầu'}
                                                    >
                                                        {review.isPinned ? <TbPinFilled size={18} /> : <TbPin size={18} />}
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {articleComments.length === 0 ? (
                                        <tr><td colSpan="4" className="text-center p-8 text-slate-500">Chưa có bình luận nào.</td></tr>
                                    ) : articleComments.map((comment) => {
                                        const displayAvatar = (comment.user && comment.user.avatar) ? comment.user.avatar : comment.avatar;
                                        return (
                                        <tr key={comment._id} className={`border-b border-slate-50 hover:bg-slate-50 transition-colors ${comment.isHidden ? 'opacity-60 bg-slate-100/50' : ''}`}>
                                            <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                    <img src={displayAvatar ? getImageUrl(displayAvatar) : `https://ui-avatars.com/api/?name=${comment.name}&background=E0F2FE&color=0284C7`} className="w-10 h-10 rounded-full object-cover shadow-sm" alt={comment.name} />
                                                    <div>
                                                        <p className="font-bold text-slate-800 text-sm">{comment.name}</p>
                                                        <p className="text-xs text-slate-400">{new Date(comment.createdAt).toLocaleDateString('vi-VN')} - {new Date(comment.createdAt).toLocaleTimeString('vi-VN', {hour: '2-digit', minute:'2-digit'})}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-4">
                                            ...
                                            <td className="p-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm font-semibold text-sky-600 truncate max-w-[120px]" title={comment.article.title}>{comment.article.title}</span>
                                                </div>
                                            </td>
                                            <td className="p-4">
                                                <div className="flex items-center justify-center gap-2">
                                                    <button 
                                                        ...
                                                        className={`p-2 rounded-lg transition-colors ${comment.isHidden ? 'bg-slate-200 text-slate-600 hover:bg-slate-300' : 'bg-sky-50 text-sky-600 hover:bg-sky-100'}`}
                                                        title={comment.isHidden ? 'Hiện bình luận' : 'Ẩn bình luận'}
                                                    >
                                                        {comment.isHidden ? <FiEyeOff size={18} /> : <FiEye size={18} />}
                                                    </button>
                                                    
                                                    <button 
                                                        onClick={() => handleToggleArticleCommentPin(comment.article._id, comment._id)}
                                                        className={`p-2 rounded-lg transition-colors ${comment.isPinned ? 'bg-amber-100 text-amber-600 hover:bg-amber-200' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                                                        title={comment.isPinned ? 'Bỏ ghim' : 'Ghim lên đầu'}
                                                    >
                                                        {comment.isPinned ? <TbPinFilled size={18} /> : <TbPin size={18} />}
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
