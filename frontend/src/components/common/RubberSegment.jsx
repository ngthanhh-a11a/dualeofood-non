...
export default function RubberSegment({
  ...
  const matchedIndex = list.findIndex(item => item.value === current);
  const hasSelection = matchedIndex !== -1;
  const index = hasSelection ? matchedIndex : (value !== undefined ? -1 : 0);
  ...
  const travel = (from, to) => {
    const a = from >= 0 ? slots.current[from] : null;
    const b = slots.current[to];
    if (!b) return;
    clearTimeout(handoff.current);
    gen.current += 1;
  ...
  const handlePointerDown = (e, i) => {
    ...
    const x = localX(e);
    const onThumb = draggable && index >= 0 && x >= edgeL.get() && x <= edgeR.get();
  ...
  const handleKeyDown = e => {
    ...
    let next = null;
    if (e.key === 'ArrowRight' || e.key === 'ArrowDown') next = index < 0 ? 0 : Math.min(last, index + 1);
    else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') next = index < 0 ? 0 : Math.max(0, index - 1);
